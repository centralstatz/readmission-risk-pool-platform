...
test_files <- sort(list.files(
  file.path(repository_root, "tests", "phase4"),
  pattern = "^test-.*[.]R$", full.names = TRUE
))
if (length(test_files) == 0L) stop("No Phase 4 test files found.", call. = FALSE)

cases <- list()
for (test_file in test_files) {
  environment <- new.env(parent = globalenv())
  sys.source(test_file, envir = environment)
  if (!exists("phase4_test_cases", envir = environment, inherits = FALSE)) {
    stop("Test file does not define phase4_test_cases(): ", test_file, call. = FALSE)
  }
  cases <- c(cases, environment$phase4_test_cases(repository_root))
}
